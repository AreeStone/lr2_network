...
def _calc_paths(source):
  """
  Dijkstra's Algorithm for finding shortest paths from a given source node.
  """
  ...
  # Initialize the path_map for all pairs
  for k in sws:
    for j, port in adjacency[k].items():
      if port is None: continue
      path_map[k][j] = (float('inf'), None)  # (distance, intermediate)
    path_map[k][k] = (0, None)  # Distance to self is 0

  # Dijkstra's algorithm
  distances = {k: float('inf') for k in sws}
  distances[source] = 0
  path_map[source][source] = (0, None)
  ...
  while priority_queue:
    ...
    for neighbor, port in adjacency[current_node].items():
      if port is None: continue
      ...
      if distance < distances[neighbor]:
        ...
        path_map[current_node][neighbor] = (distance, current_node)
        heapq.heappush(priority_queue, (distance, neighbor))

  # Optionally dump the path_map for debugging
  # dump()
